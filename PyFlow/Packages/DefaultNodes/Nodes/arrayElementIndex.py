...
class arrayElementIndex(NodeBase):
    def __init__(self, name):
        super(arrayElementIndex, self).__init__(name)
    ...
    def compute(self, *args, **kwargs):
        if self.element.getData() in self.ls.getData():
            self.result.setData(True)
            self.number.setData(self.ls.getData().index(self.element.getData()))
        else:
            self.result.setData(False)
            self.number.setData(-1)
        self.outExec.call()